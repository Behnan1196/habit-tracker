...
export type EditableItem = { id: string; group_id: string | null; kind: ItemKind; name: string; description: string | null; color: string | null; metric_unit: string | null; metric_period: 'daily' | 'weekly' | 'monthly' | null; activity_tag: string | null; estimated_minutes: number | null; is_in_plan: boolean };
  ...
  return <div className={styles.overlay} onMouseDown={onClose}><form className={styles.modal} onSubmit={submit} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="editor-title">
    ...
    <div className={styles.modalBody}>
      ...
      <div className={styles.fieldGrid}>
        {mode !== 'metric' && <div><label className={styles.label} htmlFor="item-group">{mode === 'group' ? 'Üst grup' : 'Grup'}</label><select id="item-group" className={styles.select} value={groupId ?? ''} onChange={(event) => setGroupId(event.target.value || null)}><option value="">{mode === 'group' ? 'Ana seviye' : 'Grupsuz'}</option>{groupOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></div>}
        ...
        {mode === 'activity' && <><div><label className={styles.label} htmlFor="activity-tag">Kategori / Etiket</label><input id="activity-tag" className={styles.input} list="activity-tags" value={activityTag} onChange={(event) => setActivityTag(event.target.value)} placeholder="Örn. İlaç, Egzersiz" /><datalist id="activity-tags">{activityTags.map((tag) => <option key={tag} value={tag} />)}</datalist></div><div><label className={styles.label} htmlFor="estimated-minutes">Tahmini süre</label><div className={styles.unitInput}><input id="estimated-minutes" className={styles.input} type="number" min="5" step="5" value={estimatedMinutes} onChange={(event) => setEstimatedMinutes(event.target.value)} placeholder="45" /><span>dk</span></div></div></>}
      </div>
