...
export function ItemEditorModal({ item, group, initialGroupId, initialKind, initialIsInPlan = true, groups, reminders, activityTags, onClose, onSave, onSaveGroup, onDelete }: { item?: EditableItem; group?: EditableGroup; initialGroupId: string | null; initialKind?: EditorKind; initialIsInPlan?: boolean; groups: GroupOption[]; reminders: ReminderDraft[]; activityTags: string[]; onClose: () => void; onSave: (draft: ItemDraft, reminders: ReminderDraft[]) => Promise<void>; onSaveGroup: (draft: GroupDraft) => Promise<void>; onDelete?: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (mode === 'group') await onSaveGroup({ name: name.trim(), parent_id: groupId, color, background_color: backgroundColor, content_type: group?.content_type ?? 'standard', default_item_kind: null, default_time_slot_id: null, module_key: group?.module_key ?? null, module_settings: group?.module_settings ?? {}, is_in_plan: isInPlan });
  ...
  function formatDescription(before: string, after = before, placeholder = 'metin') { const field = descriptionRef.current; if (!field) return; const start = field.selectionStart; const end = field.selectionEnd; const selected = description.slice(start, end) || placeholder; const next = `${description.slice(0, start)}${before}${selected}${after}${description.slice(end)}`; setDescription(next); queueMicrotask(() => { field.focus(); field.setSelectionRange(start + before.length, start + before.length + selected.length); }); }
  function insertLink(video = false) { const url = window.prompt(video ? 'YouTube URL’si' : 'Bağlantı URL’si'); if (!url?.trim()) return; if (video) { const field = descriptionRef.current; const start = field?.selectionStart ?? description.length; setDescription(`${description.slice(0, start)}\n${url.trim()}\n${description.slice(start)}`); } else formatDescription('[', `](${url.trim()})`, 'bağlantı metni'); }
  return <div className={styles.overlay} onMouseDown={onClose}><form className={styles.modal} onSubmit={submit} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="editor-title">
    ...
    <div className={styles.modalBody}>
      ...
      {mode !== 'group' && <><label className={styles.label} htmlFor="item-description">Açıklama / Talimat</label><div className={styles.markdownEditor}><div className={styles.markdownToolbar}><button type="button" title="Başlık" onClick={() => formatDescription('## ', '', 'Başlık')}>H</button><button type="button" title="Kalın" onClick={() => formatDescription('**', '**')}>B</button><button type="button" title="İtalik" onClick={() => formatDescription('*', '*')}><i>I</i></button><button type="button" title="Liste" onClick={() => formatDescription('- ', '', 'Liste maddesi')}>•</button><button type="button" title="Bağlantı" onClick={() => insertLink(false)}>🔗</button><button type="button" title="YouTube videosu" onClick={() => insertLink(true)}>▶</button><small>Markdown</small></div><textarea ref={descriptionRef} id="item-description" className={styles.textarea} value={description} onChange={(event) => setDescription(event.target.value)} rows={7} placeholder={'## Ne işe yarar?\n\nAçıklama veya uygulama talimatı…\n\nhttps://youtu.be/...'} /></div></>}
