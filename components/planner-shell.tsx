...
type ScheduleRow = { id: string; item_id: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; end_date: string | null; is_active: boolean; reminder_time: string | null };
...
export function PlannerShell({ user }: { user: User }) {
  ...
  async function saveSchedule(draft: { id?: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; reminder_time: string | null }) {
...
function ScheduleManagerModal({ item, schedules, slots, selectedDate, onClose, onSave, onToggle, onDelete }: { item: ItemRow; schedules: ScheduleRow[]; slots: SlotRow[]; selectedDate: string; onClose: () => void; onSave: (draft: { id?: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; reminder_time: string | null }) => Promise<boolean>; onToggle: (schedule: ScheduleRow) => Promise<void>; onDelete: (schedule: ScheduleRow) => Promise<void> }) {
  ...
  return <div className={styles.overlay} onMouseDown={onClose}><section className={`${styles.dialog} ${styles.scheduleDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>Ajanda programları</span><button onClick={onClose}>×</button></div><h2>{item.name}</h2>{editing ? <ScheduleEditor schedule={editing === 'new' ? undefined : editing} slots={slots} selectedDate={selectedDate} onCancel={schedules.length ? () => setEditing(null) : onClose} onSave={async (draft) => { const saved = await onSave(draft); if (saved) setEditing(null); return saved; }} /> : <><p>Aktivitenin Ajandaya bağlı tekrarlarını yönet.</p><div className={styles.scheduleList}>{schedules.map((schedule) => <article key={schedule.id} className={!schedule.is_active ? styles.schedulePaused : ''}><button onClick={() => setEditing(schedule)}><i style={{ background: slots.find((slot) => slot.id === schedule.time_slot_id)?.color ?? palette[0] }} /><span><strong>{slots.find((slot) => slot.id === schedule.time_slot_id)?.name ?? 'Zaman dilimi'}</strong><small>{describe(schedule)}{schedule.reminder_time ? ` · 🔔 ${schedule.reminder_time.slice(0, 5)}` : ''}</small></span></button><div><button onClick={() => void onToggle(schedule)}>{schedule.is_active ? 'Duraklat' : 'Başlat'}</button><button onClick={() => void onDelete(schedule)}>Sil</button></div></article>)}</div><button className={styles.primary} onClick={() => setEditing('new')}>＋ Yeni zaman ekle</button></>}</section></div>;
}

function ScheduleEditor({ schedule, slots, selectedDate, onCancel, onSave }: { schedule?: ScheduleRow; slots: SlotRow[]; selectedDate: string; onCancel: () => void; onSave: (draft: { id?: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; reminder_time: string | null }) => Promise<boolean> }) {
  ...
  const labels = ['Pz', 'Pt', 'Sa', 'Ça', 'Pe', 'Cu', 'Ct'];
  async function submit() { setBusy(true); await onSave({ id: schedule?.id, time_slot_id: slotId, recurrence_type: recurrence, weekdays: recurrence === 'weekdays' ? weekdays : [], start_date: startDate, reminder_time: reminderTime || null }); setBusy(false); }
  return <><p>{schedule ? 'Programı düzenle.' : 'Yeni bir tekrar ve zaman dilimi ekle.'}</p><label className={styles.scheduleLabel}>Başlangıç tarihi</label><input className={styles.scheduleDate} type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /><label className={styles.scheduleLabel}>Tekrar</label><div className={styles.scheduleRecurrence}><button className={recurrence === 'once' ? styles.chosen : ''} onClick={() => setRecurrence('once')}>Yalnız bu gün</button><button className={recurrence === 'daily' ? styles.chosen : ''} onClick={() => setRecurrence('daily')}>Her gün</button><button className={recurrence === 'weekdays' ? styles.chosen : ''} onClick={() => setRecurrence('weekdays')}>Belirli günler</button></div>{recurrence === 'weekdays' && <div className={styles.scheduleWeekdays}>{labels.map((label, day) => <button key={label} className={weekdays.includes(day) ? styles.chosen : ''} onClick={() => setWeekdays((current) => current.includes(day) ? current.filter((value) => value !== day) : [...current, day].sort())}>{label}</button>)}</div>}<label className={styles.scheduleLabel}>Zaman dilimi</label><div className={styles.slotPicker}>{slots.map((slot) => <button key={slot.id} className={slotId === slot.id ? styles.chosen : ''} onClick={() => setSlotId(slot.id)}><i style={{ background: slot.color ?? palette[0] }} /><span><strong>{slot.name}</strong><small>{shortTime(slot.start_time)}–{shortTime(slot.end_time)}</small></span><b>{slotId === slot.id ? '✓' : ''}</b></button>)}</div><label className={styles.scheduleReminder}><span>Bildirim saati</span><input type="time" value={reminderTime} onChange={(event) => setReminderTime(event.target.value)} /><small>Boş bırakırsan bildirim gönderilmez.</small></label><div className={styles.scheduleFormActions}><button onClick={onCancel}>Vazgeç</button><button className={styles.primary} disabled={busy || !startDate || !slotId || (recurrence === 'weekdays' && !weekdays.length)} onClick={() => void submit()}>{busy ? 'Kaydediliyor…' : 'Kaydet'}</button></div></>;
}
