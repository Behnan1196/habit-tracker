...
function recurrenceLabel(schedule?: ScheduleRow) {
  if (!schedule) return 'Yalnız bugün';
  if (schedule.recurrence_type === 'daily') return 'Her gün';
  if (schedule.recurrence_type === 'once') return 'Yalnız bugün';
...
export function PlannerShell({ user }: { user: User }) {
  ...
  const [agendaBlockEditor, setAgendaBlockEditor] = useState<AgendaBlockRow | null>(null);
  const [agendaRemoveTarget, setAgendaRemoveTarget] = useState<{ entry: AssignmentRow; item: ItemRow; schedule?: ScheduleRow } | null>(null);
  ...
  const [agendaScrollPosition, setAgendaScrollPosition] = useState(0);
  ...
  async function addLibraryItemToAgenda(item: ItemRow) {
    if (!preferredSlotId || item.kind === 'metric') return;
    const slotId = preferredSlotId;
    const blockId = preferredBlockId && agendaBlocks.some((block) => block.id === preferredBlockId && block.time_slot_id === slotId) ? preferredBlockId : null;
    const positions = [...schedules.filter((schedule) => schedule.time_slot_id === slotId).map((schedule) => schedule.agenda_position), ...todoTasks.filter((task) => task.time_slot_id === slotId).map((task) => task.agenda_position), ...agendaBlocks.filter((block) => block.time_slot_id === slotId).map((block) => block.agenda_position)];
    const agendaPosition = (positions.length ? Math.max(...positions) : -1) + 1;
    const existing = schedules.find((schedule) => schedule.item_id === item.id && schedule.time_slot_id === slotId);
    const result = existing
      ? await supabase.from('m_agenda_schedules').update({ block_id: blockId, recurrence_type: 'daily', weekdays: [], start_date: selectedDate < existing.start_date ? selectedDate : existing.start_date, end_date: null, is_active: true, agenda_position: agendaPosition }).eq('id', existing.id)
      : await supabase.from('m_agenda_schedules').insert({ user_id: user.id, item_id: item.id, time_slot_id: slotId, block_id: blockId, recurrence_type: 'daily', weekdays: [], start_date: selectedDate, end_date: null, is_active: true, reminder_time: null, scheduled_time: null, agenda_position: agendaPosition });
    if (result.error) { setError(result.error.message); return; }
    const cleared = await supabase.from('m_daily_assignments').delete().eq('item_id', item.id).eq('time_slot_id', slotId).gte('plan_date', selectedDate).eq('status', 'cancelled');
    if (cleared.error) { setError(cleared.error.message); return; }
    setPreferredSlotId(null); setPreferredBlockId(null); setWorkspace('agenda'); window.history.replaceState(null, '', '/'); await loadData(); restoreAgendaScroll();
  }

  function openLibraryPicker(slotId: string, blockId: string | null, date: string) {
    setAgendaScrollPosition(window.scrollY);
    setPreferredSlotId(slotId); setPreferredBlockId(blockId); setSelectedDate(date); setAgendaAddTarget(null); setWorkspace('library');
  }

  function restoreAgendaScroll() {
    requestAnimationFrame(() => requestAnimationFrame(() => window.scrollTo(0, agendaScrollPosition)));
  }

  function cancelLibraryPicker() {
    setPreferredSlotId(null); setPreferredBlockId(null); setWorkspace('agenda'); restoreAgendaScroll();
  }

  async function removeAgendaFromDate(target: { entry: AssignmentRow; schedule?: ScheduleRow }) {
    if (!target.schedule) { await cancelAssignment(target.entry); setAgendaRemoveTarget(null); return; }
    const schedule = target.schedule; const cutoff = target.entry.plan_date;
    const removed = await supabase.from('m_daily_assignments').delete().eq('schedule_id', schedule.id).gte('plan_date', cutoff).eq('status', 'planned');
    if (removed.error) return setError(removed.error.message);
    if (cutoff <= schedule.start_date) {
      const deleted = await supabase.from('m_agenda_schedules').delete().eq('id', schedule.id);
      if (deleted.error) return setError(deleted.error.message);
    } else {
      const endDate = isoDate(addDays(parseDate(cutoff), -1));
      const stopped = await supabase.from('m_agenda_schedules').update({ end_date: endDate }).eq('id', schedule.id);
      if (stopped.error) return setError(stopped.error.message);
    }
    const detached = await supabase.from('m_daily_assignments').delete().eq('item_id', schedule.item_id).eq('time_slot_id', schedule.time_slot_id).is('schedule_id', null).gte('plan_date', cutoff).eq('status', 'planned');
    if (detached.error) return setError(detached.error.message);
    setAgendaRemoveTarget(null); await loadData();
  }
  ...
  async function renameAgendaBlock(block: AgendaBlockRow, name: string) {
    const result = await supabase.from('m_agenda_blocks').update({ name }).eq('id', block.id);
    if (result.error) { setError(result.error.message); return false; }
    setAgendaBlockEditor(null); await loadData(); return true;
  }

  async function deleteAgendaBlock(block: AgendaBlockRow) {
    if (!window.confirm(`“${block.name}” bloğu silinsin mi? İçindeki aktiviteler zaman diliminde kalır.`)) return false;
    const result = await supabase.from('m_agenda_blocks').delete().eq('id', block.id);
    if (result.error) { setError(result.error.message); return false; }
    setCollapsedBlocks((current) => { const next = new Set(current); next.delete(block.id); return next; });
    setAgendaBlockEditor(null); await loadData(); return true;
  }
  ...
  function renderLibraryItem(item: ItemRow) {
    const hasReminder = reminders.some((reminder) => reminder.item_id === item.id && reminder.is_enabled);
    const itemSchedules = schedules.filter((schedule) => schedule.item_id === item.id && schedule.is_active);
    const location = (schedule: ScheduleRow) => { const slot = slots.find((candidate) => candidate.id === schedule.time_slot_id)?.name ?? 'Zaman dilimi'; const block = schedule.block_id ? agendaBlocks.find((candidate) => candidate.id === schedule.block_id)?.name : null; return block ? `${slot} · ${block}` : slot; };
    const locationSummary = itemSchedules.length ? `${location(itemSchedules[0])}${itemSchedules.length > 1 ? ` +${itemSchedules.length - 1}` : ''}` : '';
    return <div className={`${styles.libraryRow} ${preferredSlotId ? styles.librarySelectableRow : ''}`} style={{ marginLeft: 12 }} key={item.id} draggable={!preferredSlotId} onDragStart={(event) => { event.stopPropagation(); event.dataTransfer.setData('text/library-kind', 'item'); event.dataTransfer.setData('text/library-id', item.id); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); if (event.dataTransfer.getData('text/library-kind') === 'item') void moveBefore('item', event.dataTransfer.getData('text/library-id'), item.id); }}>
      <button className={styles.libraryItemIdentity} style={{ paddingLeft: 8 }} onClick={() => preferredSlotId ? void addLibraryItemToAgenda(item) : setEditor({ item, groupId: item.group_id })}><span className={styles.libraryDrag}>{preferredSlotId ? '＋' : '⠿'}</span><i style={{ background: item.color ?? palette[0] }} /><span><strong>{item.name}</strong><small>{item.kind === 'metric' ? `Ölçüm${item.metric_unit ? ` · ${item.metric_unit}` : ''}` : item.activity_tags.length ? item.activity_tags.join(' · ') : 'İşlev belirtilmedi'}{item.estimated_minutes ? ` · ${item.estimated_minutes} dk` : ''}{hasReminder ? ' · 🔔' : ''}</small></span></button>
      {!preferredSlotId && itemSchedules.length > 0 && <div className={styles.libraryRowActions}><button className={styles.libraryAgendaLocation} title="Ajanda ayarlarını aç" onClick={() => setScheduleTarget(item)}>{locationSummary}</button></div>}
    </div>;
  }

  function renderLibraryGroup(group: GroupRow): React.ReactNode {
    ...
    return <section className={styles.libraryGroup} key={group.id} draggable onDragStart={(event) => { event.stopPropagation(); event.dataTransfer.setData('text/library-kind', 'group'); event.dataTransfer.setData('text/library-id', group.id); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); const kind = event.dataTransfer.getData('text/library-kind'); const id = event.dataTransfer.getData('text/library-id'); if (kind === 'item') void moveItem(id, group.id); else if (kind === 'group') void moveGroup(id, group.id); }}>
      <header style={{ paddingLeft: 10, background: group.background_color ?? '#f4f5f1', color: readableText(group.background_color ?? '#f4f5f1') }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); const kind = event.dataTransfer.getData('text/library-kind'); const id = event.dataTransfer.getData('text/library-id'); if (kind === 'group') void moveBefore('group', id, group.id); else if (kind === 'item') void moveItem(id, group.id); }}><span className={styles.libraryDrag}>⠿</span><i style={{ background: group.color ?? palette[0] }} /><button className={styles.groupTitle} aria-expanded={!collapsed} onClick={() => setCollapsedLibraryGroups((current) => { const next = new Set(current); if (next.has(group.id)) next.delete(group.id); else next.add(group.id); return next; })}>{group.name}</button><small>{groupItems.length + children.length}</small>{!preferredSlotId && <div><button aria-label="Grubu düzenle" title="Grubu düzenle" onClick={() => void editGroup(group)}>…</button><button title="Gruba ekle" onClick={() => setLibraryAddTarget(group)}>＋</button></div>}</header>
      {!collapsed && <>{groupItems.map((item) => renderLibraryItem(item))}{children.map((child) => renderLibraryGroup(child))}</>}
    </section>;
  }
  ...
  return <div className={styles.app}>
    ...
    {agendaItemTarget && <AgendaItemModal item={agendaItemTarget} group={groups.find((group) => group.id === agendaItemTarget.group_id)} date={selectedDate} reminders={reminders.filter((reminder) => reminder.item_id === agendaItemTarget.id && reminder.is_enabled)} schedules={schedules.filter((schedule) => schedule.item_id === agendaItemTarget.id && schedule.is_active)} slots={activeSlots} blocks={agendaBlocks} onClose={() => setAgendaItemTarget(null)} onManageProgram={() => { const item = agendaItemTarget; setAgendaItemTarget(null); setScheduleTarget(item); }} onEditSource={() => { const item = agendaItemTarget; setAgendaItemTarget(null); setEditor({ item, groupId: item.group_id }); }} />}
    ...
    {agendaBlockEditor && <AgendaBlockEditorModal block={agendaBlockEditor} onClose={() => setAgendaBlockEditor(null)} onSave={(name) => renameAgendaBlock(agendaBlockEditor, name)} onDelete={() => deleteAgendaBlock(agendaBlockEditor)} />}
    {agendaRemoveTarget && <div className={styles.overlay} onMouseDown={() => setAgendaRemoveTarget(null)}><section className={`${styles.dialog} ${styles.agendaRemoveDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>Ajandadan kaldır</span><button onClick={() => setAgendaRemoveTarget(null)}>×</button></div><h2>{agendaRemoveTarget.item.name}</h2><p>{agendaRemoveTarget.schedule ? 'Bu aktivitenin tekrar eden programını nasıl değiştirmek istiyorsun?' : 'Bu aktivite yalnız seçili günden kaldırılabilir.'}</p><div className={styles.agendaRemoveOptions}><button onClick={async () => { await cancelAssignment(agendaRemoveTarget.entry); setAgendaRemoveTarget(null); }}><strong>Yalnız bu gün kaldır</strong><small>Diğer günlerde görünmeye devam eder.</small></button>{agendaRemoveTarget.schedule && <button className={styles.agendaRemoveFuture} onClick={() => void removeAgendaFromDate(agendaRemoveTarget)}><strong>Bu günden itibaren kaldır</strong><small>Geçmiş ve tamamlanan kayıtlar korunur.</small></button>}<button onClick={() => setAgendaRemoveTarget(null)}><strong>Vazgeç</strong></button></div></section></div>}
    {agendaAddTarget && <div className={styles.overlay} onMouseDown={() => setAgendaAddTarget(null)}><section className={`${styles.dialog} ${styles.agendaAddDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>{agendaAddTarget.slot.name}</span><button onClick={() => setAgendaAddTarget(null)}>×</button></div><h2>Ajandaya ekle</h2><p>Bu zaman dilimine ne eklemek istiyorsun?</p><div className={styles.agendaAddOptions}><button onClick={() => openLibraryPicker(agendaAddTarget.slot.id, null, agendaAddTarget.date)}><b>▦</b><span><strong>Kütüphaneden aktivite</strong><small>Zaman dilimi otomatik seçilecek</small></span></button><button onClick={() => void createAgendaBlock(agendaAddTarget.slot)}><b>▤</b><span><strong>Blok oluştur</strong><small>İlaçlar veya Sabah rutini gibi</small></span></button></div></section></div>}
    ...
    {editor && <ItemEditorModal key={editor.item?.id ?? editor.group?.id ?? `new-${editor.initialKind ?? 'item'}-${editor.groupId ?? 'root'}`} item={editor.item} group={editor.group} initialKind={editor.initialKind} initialIsInPlan={editor.initialIsInPlan} initialGroupId={editor.groupId} groups={groups} reminders={editor.item ? reminders.filter((reminder) => reminder.item_id === editor.item!.id).map(({ reminder_time, weekdays, is_enabled }) => ({ reminder_time: reminder_time.slice(0, 5), weekdays, is_enabled })) : []} activityTags={Array.from(new Set(items.map((item) => item.activity_tag).filter((tag): tag is string => !!tag))).sort((a, b) => a.localeCompare(b, 'tr'))} onClose={() => setEditor(null)} onSave={saveItem} onSaveGroup={saveGroup} onDelete={editor.item ? async () => { if (await deleteItem(editor.item!)) setEditor(null); } : editor.group ? async () => { if (await deleteGroup(editor.group!)) setEditor(null); } : undefined} />}
  </div>;
}

function AgendaBlockEditorModal({ block, onClose, onSave, onDelete }: { block: AgendaBlockRow; onClose: () => void; onSave: (name: string) => Promise<boolean>; onDelete: () => Promise<boolean> }) {
  const [name, setName] = useState(block.name);
  const [busy, setBusy] = useState(false);
  async function submit() { const value = name.trim(); if (!value) return; setBusy(true); await onSave(value); setBusy(false); }
  return <div className={styles.overlay} onMouseDown={onClose}><section className={`${styles.dialog} ${styles.blockEditorDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>Bloğu düzenle</span><button onClick={onClose}>×</button></div><h2>{block.name}</h2><label className={styles.blockEditorLabel}>Blok adı<input autoFocus value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void submit(); }} /></label><div className={styles.blockEditorActions}><button className={styles.blockDelete} disabled={busy} onClick={() => void onDelete()}>Bloğu sil</button><button disabled={busy || !name.trim()} onClick={() => void submit()}>{busy ? 'Kaydediliyor…' : 'Kaydet'}</button></div><small className={styles.blockEditorNote}>Blok silinirse içindeki aktiviteler Ajandadan silinmez; aynı zaman diliminde grupsuz olarak kalır.</small></section></div>;
}
...
function AgendaItemModal({ item, group, date, reminders, schedules, slots, blocks, onClose, onManageProgram, onEditSource }: { item: ItemRow; group?: GroupRow; date: string; reminders: ReminderRow[]; schedules: ScheduleRow[]; slots: SlotRow[]; blocks: AgendaBlockRow[]; onClose: () => void; onManageProgram: () => void; onEditSource: () => void }) {
  return <div className={styles.overlay} onMouseDown={onClose}><section className={`${styles.dialog} ${styles.agendaItemDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>{new Intl.DateTimeFormat('tr-TR', { dateStyle: 'long' }).format(parseDate(date))}</span><button onClick={onClose}>×</button></div><h2>{item.name}</h2><div className={styles.agendaSourceCard}><span>Kütüphane</span><strong>{group?.name ?? 'Grupsuz'}</strong><small>{item.activity_tag || 'Aktivite'}{item.estimated_minutes ? ` · ${item.estimated_minutes} dk` : ''}{reminders.length ? ' · 🔔' : ''}</small></div>{item.description && <section className={styles.agendaDescription}><MarkdownContent value={item.description} /></section>}<section className={styles.agendaProgramSummary}><span>Ajandada</span>{schedules.length ? schedules.map((schedule) => { const slot = slots.find((candidate) => candidate.id === schedule.time_slot_id); const block = blocks.find((candidate) => candidate.id === schedule.block_id); return <div key={schedule.id}><strong>{slot?.name ?? 'Zaman dilimi'}{block ? ` · ${block.name}` : ''}</strong><small>{recurrenceLabel(schedule)}{schedule.scheduled_time ? ` · ${schedule.scheduled_time.slice(0, 5)}` : ''}{schedule.reminder_time ? ` · 🔔 ${schedule.reminder_time.slice(0, 5)}` : ''}</small></div>; }) : <div><strong>Yalnız bugün</strong><small>Tekrarlayan programı yok.</small></div>}</section><div className={styles.agendaItemActions}><button onClick={onManageProgram}>Ajanda ayarları</button><button onClick={onEditSource}>Kütüphane kaydını düzenle</button></div></section></div>;
}
