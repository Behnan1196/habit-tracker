...
type ScheduleRow = { id: string; item_id: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; end_date: string | null; is_active: boolean; reminder_time: string | null; scheduled_time: string | null; agenda_position: number };
type ScheduleDraft = { id?: string; time_slot_id: string; recurrence_type: 'once' | 'daily' | 'weekdays'; weekdays: number[]; start_date: string; reminder_time: string | null; scheduled_time: string | null; agenda_position: number };
type AgendaOrderRow = { key: string; kind: 'activity' | 'todo'; scheduledTime: string | null; position: number; schedule?: ScheduleRow; task?: TodoTaskRow };
...
export function PlannerShell({ user }: { user: User }) {
  ...
  const loadData = useCallback(async () => {
    const [groupResult, itemResult, slotResult, assignmentResult, persistentResult, metricResult, noteResult, reminderResult, scheduleResult, todoListResult, todoTaskResult] = await Promise.all([
      ...
      supabase.from('m_agenda_schedules').select('id,item_id,time_slot_id,recurrence_type,weekdays,start_date,end_date,is_active,reminder_time,scheduled_time,agenda_position').order('created_at'),
  ...
  function moveAgendaRow(rows: AgendaOrderRow[], index: number, direction: -1 | 1) {
    const target = index + direction;
    if (target < 0 || target >= rows.length) return;
    if ((rows[index].scheduledTime ?? '') !== (rows[target].scheduledTime ?? '')) {
      setError('Kesin saatli kayıtların sırası saatlerine göre belirlenir. Saati düzenleyerek yerini değiştirebilirsin.');
      return;
    }
    const ordered = [...rows];
  ...
  async function moveLibraryItem(item: ItemRow, direction: -1 | 1) {
    const siblings = items.filter((candidate) => candidate.group_id === item.group_id && candidate.kind !== 'metric').sort((a, b) => a.position - b.position);
    const index = siblings.findIndex((candidate) => candidate.id === item.id);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= siblings.length) return;
    [siblings[index], siblings[target]] = [siblings[target], siblings[index]];
    const results = await Promise.all(siblings.map((candidate, position) => supabase.from('m_items').update({ position }).eq('id', candidate.id)));
    const failed = results.find((result) => result.error)?.error;
    if (failed) setError(failed.message); else await loadData();
  }
  ...
  function renderLibraryItem(item: ItemRow, depth = 0) {
    ...
    return <div className={styles.libraryRow} key={item.id} draggable onDragStart={(event) => { event.dataTransfer.setData('text/library-kind', 'item'); event.dataTransfer.setData('text/library-id', item.id); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); if (event.dataTransfer.getData('text/library-kind') === 'item') void moveBefore('item', event.dataTransfer.getData('text/library-id'), item.id); }}>
      ...
      <div className={styles.libraryRowActions}><div className={styles.libraryOrder}><button aria-label="Yukarı taşı" disabled={siblingIndex <= 0} onClick={() => void moveLibraryItem(item, -1)}>↑</button><button aria-label="Aşağı taşı" disabled={siblingIndex < 0 || siblingIndex === siblings.length - 1} onClick={() => void moveLibraryItem(item, 1)}>↓</button></div>{item.kind !== 'metric' && <button className={schedules.some((schedule) => schedule.item_id === item.id) ? styles.libraryInPlan : ''} onClick={() => setScheduleTarget(item)}>{schedules.some((schedule) => schedule.item_id === item.id) ? 'Programlı ✓' : 'Ajandaya ekle'}</button>}</div>
    </div>;
  }
  ...
  function renderAgendaDay(date: Date) {
    ...
    return <section className={styles.agendaDay} key={key}><header><div><span>{new Intl.DateTimeFormat('tr-TR', { weekday: 'long' }).format(date)}</span><strong>{new Intl.DateTimeFormat('tr-TR', { day: 'numeric', month: 'long' }).format(date)}</strong></div>{view === 'weekly' && <button onClick={() => { setSelectedDate(key); setView('daily'); }}>Günü aç</button>}</header><div className={styles.agendaSlots}>{activeSlots.map((slot) => {
      ...
      const orderRows: AgendaOrderRow[] = [...activityRows.map((row) => ({ key: `activity-${row.entry.id}`, kind: 'activity' as const, scheduledTime: row.schedule?.scheduled_time ?? null, position: row.schedule?.agenda_position ?? 0, schedule: row.schedule })), ...todoEntries.map((task) => ({ key: `todo-${task.id}`, kind: 'todo' as const, scheduledTime: task.scheduled_time, position: task.agenda_position, task }))].sort((a, b) => {
      ...
      return <section className={styles.agendaSlotSection} key={slot.id}><header><i style={{ background: slot.color ?? palette[0] }} /><div><strong>{slot.name}</strong><small>{shortTime(slot.start_time)}–{shortTime(slot.end_time)}</small></div><span>{orderRows.length}</span></header><div>{orderRows.map((row, index) => {
        const activity = row.kind === 'activity' ? activityRows.find((candidate) => `activity-${candidate.entry.id}` === row.key) : undefined;
        const task = row.task;
        if (activity) return <article draggable className={activity.entry.status === 'done' ? styles.agendaEntryDone : ''} key={row.key} onDragStart={(event) => event.dataTransfer.setData('text/agenda-row', row.key)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); dropAgendaRow(orderRows, event.dataTransfer.getData('text/agenda-row'), row.key); }}><button className={styles.agendaEntryState} onClick={() => { setSelectedDate(key); if (activity.entry.status === 'done') void restoreAssignment({ ...activity.entry, source: 'daily' }); else if (activity.item.activity_tag || activity.item.estimated_minutes) setDurationTarget({ ...activity.entry, source: 'daily' }); else void completeAssignment({ ...activity.entry, source: 'daily' }); }}><span>{activity.entry.status === 'done' ? '✓' : ''}</span></button><button className={styles.agendaEntryIdentity} onClick={() => { setSelectedDate(key); setAgendaItemTarget(activity.item); }}><strong>{activity.item.name}</strong><small>{row.scheduledTime ? `${row.scheduledTime.slice(0, 5)} · ` : ''}{groups.find((group) => group.id === activity.item.group_id)?.name ?? activity.item.activity_tag ?? 'Aktivite'}{activity.item.estimated_minutes ? ` · ${activity.item.estimated_minutes} dk` : ''}</small></button><div className={styles.agendaOrder}><button aria-label="Yukarı taşı" disabled={index === 0} onClick={() => moveAgendaRow(orderRows, index, -1)}>↑</button><button aria-label="Aşağı taşı" disabled={index === orderRows.length - 1} onClick={() => moveAgendaRow(orderRows, index, 1)}>↓</button></div><button className={styles.agendaEntryRemove} aria-label="Yalnız bu günden kaldır" title="Yalnız bu günden kaldır" onClick={() => void cancelAssignment({ ...activity.entry, source: 'daily' })}>×</button></article>;
        if (task) return <article draggable className={`${styles.agendaTodoEntry} ${task.status === 'done' ? styles.agendaEntryDone : ''}`} key={row.key} onDragStart={(event) => event.dataTransfer.setData('text/agenda-row', row.key)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); dropAgendaRow(orderRows, event.dataTransfer.getData('text/agenda-row'), row.key); }}><button className={styles.agendaEntryState} onClick={() => void toggleTodo(task)}><span>{task.status === 'done' ? '✓' : ''}</span></button><button className={styles.agendaEntryIdentity} onClick={() => { setWorkspace('modules'); setActiveModule('todo'); }}><strong>{task.title}</strong><small>{task.scheduled_time ? `${task.scheduled_time.slice(0, 5)} · ` : ''}Todo · {todoLists.find((list) => list.id === task.list_id)?.name ?? 'Gelen Kutusu'}{task.priority === 'high' ? ' · Yüksek' : ''}</small></button><div className={styles.agendaOrder}><button aria-label="Yukarı taşı" disabled={index === 0} onClick={() => moveAgendaRow(orderRows, index, -1)}>↑</button><button aria-label="Aşağı taşı" disabled={index === orderRows.length - 1} onClick={() => moveAgendaRow(orderRows, index, 1)}>↓</button></div><button className={styles.agendaEntryRemove} aria-label="Ajandadan kaldır" onClick={() => void saveTodo({ title: task.title, description: task.description, list_id: task.list_id, priority: task.priority, agenda_date: null, time_slot_id: null, reminder_time: null, scheduled_time: null, agenda_position: task.agenda_position }, task)}>×</button></article>;
        return null;
      })}{!orderRows.length && <button className={styles.agendaSlotEmpty} onClick={() => { setSelectedDate(key); setWorkspace('library'); }}>Kütüphaneden aktivite ekle</button>}</div></section>;
    })}</div></section>;
  }
  ...
  return <div className={styles.app}>
    ...
    {scheduleTarget && <ScheduleManagerModal item={scheduleTarget} schedules={schedules.filter((schedule) => schedule.item_id === scheduleTarget.id)} slots={activeSlots} selectedDate={selectedDate} onClose={() => setScheduleTarget(null)} onSave={saveSchedule} onToggle={toggleSchedule} onDelete={deleteSchedule} />}
...
function ScheduleManagerModal({ item, schedules, slots, selectedDate, onClose, onSave, onToggle, onDelete }: { item: ItemRow; schedules: ScheduleRow[]; slots: SlotRow[]; selectedDate: string; onClose: () => void; onSave: (draft: ScheduleDraft) => Promise<boolean>; onToggle: (schedule: ScheduleRow) => Promise<void>; onDelete: (schedule: ScheduleRow) => Promise<void> }) {
  ...
  return <div className={styles.overlay} onMouseDown={onClose}><section className={`${styles.dialog} ${styles.scheduleDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>{schedules.length ? 'Ajanda programları' : 'Ajandaya ekle'}</span><button onClick={onClose}>×</button></div><h2>{item.name}</h2>{editing ? <ScheduleEditor schedule={editing === 'new' ? undefined : editing} slots={slots} selectedDate={selectedDate} nextPosition={schedules.length} onCancel={schedules.length ? () => setEditing(null) : onClose} onSave={async (draft) => { const saved = await onSave(draft); if (saved) { if (schedules.length) setEditing(null); else onClose(); } return saved; }} /> : <><p>Aktivitenin Ajandaya bağlı tekrarlarını yönet.</p><div className={styles.scheduleList}>{schedules.map((schedule) => <article key={schedule.id} className={!schedule.is_active ? styles.schedulePaused : ''}><button onClick={() => setEditing(schedule)}><i style={{ background: slots.find((slot) => slot.id === schedule.time_slot_id)?.color ?? palette[0] }} /><span><strong>{schedule.scheduled_time ? `${schedule.scheduled_time.slice(0, 5)} · ` : ''}{slots.find((slot) => slot.id === schedule.time_slot_id)?.name ?? 'Zaman dilimi'}</strong><small>{describe(schedule)}{schedule.reminder_time ? ` · 🔔 ${schedule.reminder_time.slice(0, 5)}` : ''}</small></span></button><div><button onClick={() => void onToggle(schedule)}>{schedule.is_active ? 'Duraklat' : 'Başlat'}</button><button onClick={() => void onDelete(schedule)}>Sil</button></div></article>)}</div><button className={styles.primary} onClick={() => setEditing('new')}>＋ Yeni zaman ekle</button></>}</section></div>;
}

function ScheduleEditor({ schedule, slots, selectedDate, nextPosition, onCancel, onSave }: { schedule?: ScheduleRow; slots: SlotRow[]; selectedDate: string; nextPosition: number; onCancel: () => void; onSave: (draft: ScheduleDraft) => Promise<boolean> }) {
  const [slotId, setSlotId] = useState(schedule?.time_slot_id ?? slots[0]?.id ?? '');
  ...
  const labels = ['Pz', 'Pt', 'Sa', 'Ça', 'Pe', 'Cu', 'Ct'];
  async function submit() { setBusy(true); await onSave({ id: schedule?.id, time_slot_id: slotId, recurrence_type: recurrence, weekdays: recurrence === 'weekdays' ? weekdays : [], start_date: startDate, reminder_time: reminderTime || null, scheduled_time: scheduledTime || null, agenda_position: schedule?.agenda_position ?? nextPosition }); setBusy(false); }
