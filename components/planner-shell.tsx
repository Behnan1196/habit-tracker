...
type GroupRow = { id: string; parent_id: string | null; name: string; color: string | null; background_color: string | null; position: number; content_type: 'standard' | 'module'; default_item_kind: ItemKind | null; default_time_slot_id: string | null; module_key: string | null; module_settings: Record<string, unknown> };
...
export function PlannerShell({ user }: { user: User }) {
  ...
  function renderGroup(group: GroupRow, depth = 0): React.ReactNode {
    const groupItems = items.filter((item) => item.group_id === group.id && item.is_in_plan); const children = groups.filter((candidate) => candidate.parent_id === group.id);
  ...
  async function togglePlanItem(item: ItemRow) {
    const result = await supabase.from('m_items').update({ is_in_plan: !item.is_in_plan }).eq('id', item.id);
    if (result.error) setError(result.error.message); else await loadData();
  }
  ...
  return <div className={styles.app}>
    <main className={styles.mainWide}>
      ...
      <div className={styles.stickyTop}><header className={styles.header}><div className={styles.compactBrand}><span>M</span><strong>Plan</strong></div><div className={styles.headerActions}><button className={styles.agendaButton} onClick={() => openAgenda('agenda')}><span>▤</span> Ajanda</button><button className={styles.focusButton} onClick={() => openAgenda('focus')}><span className={styles.focusDot}>●</span> Focus <b>{focusPlannedCount}</b></button><AppMenu user={user} active="calendar" view={view} onViewChange={changeView} onOpenLibrary={() => setLibraryOpen(true)} notificationPermission={notificationPermission} onEnableNotifications={() => void enableNotifications()} onManageSlots={() => setSlotManagerOpen(true)} /></div></header></div>
      <section className={`${styles.calendarBoard} ${view === 'daily' ? styles.dailyView : ''}`}>
        <div className={styles.calendarScroller}>
          ...
          {view === 'weekly' && <div className={styles.calendarHead}><div />{visibleDates.map((date) => { const key = isoDate(date); return <button key={key} className={selectedDate === key ? styles.selectedDay : ''} onClick={() => setSelectedDate(key)}><span>{new Intl.DateTimeFormat('tr-TR', { weekday: 'short' }).format(date)}</span><strong>{date.getDate()}</strong></button>; })}</div>}
          <div onDragOver={(event) => event.preventDefault()} onDrop={(event) => void moveItem(event.dataTransfer.getData('text/item-id'), null)}>{groups.filter((group) => group.parent_id === null).map((group) => renderGroup(group))}{items.filter((item) => item.group_id === null && item.is_in_plan).map((item) => renderItem(item))}{!items.some((item) => item.is_in_plan) && <div className={styles.empty}>Planın boş. Kütüphaneden bu dönem kullanmak istediğin aktiviteleri ekleyebilirsin.</div>}</div>
        </div>
      </section>
    </main>
    ...
    {libraryOpen && <LibraryModal items={items} groups={groups} reminders={reminders} onClose={() => setLibraryOpen(false)} onTogglePlan={togglePlanItem} onEdit={(item) => { setLibraryOpen(false); setEditor({ item, groupId: item.group_id }); }} onCreate={(kind) => { setLibraryOpen(false); setEditor({ groupId: null, initialKind: kind, initialIsInPlan: false }); }} onAddToday={(item) => { setLibraryOpen(false); if (item.kind === 'persistent') void quickPlanPersistent(item); else if (item.kind === 'daily') setPlanTarget({ item, date: selectedDate }); }} />}
...
function LibraryModal({ items, groups, reminders, onClose, onTogglePlan, onEdit, onCreate, onAddToday }: { items: ItemRow[]; groups: GroupRow[]; reminders: ReminderRow[]; onClose: () => void; onTogglePlan: (item: ItemRow) => Promise<void>; onEdit: (item: ItemRow) => void; onCreate: (kind: ItemKind | 'group') => void; onAddToday: (item: ItemRow) => void }) {
  const [query, setQuery] = useState('');
  const visible = items.filter((item) => item.name.toLocaleLowerCase('tr').includes(query.toLocaleLowerCase('tr')));
  return <div className={styles.overlay} onMouseDown={onClose}><section className={`${styles.dialog} ${styles.libraryDialog}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className={styles.dialogTop}><span>Tekrar kullanılabilir aktiviteler</span><button onClick={onClose}>×</button></div><div className={styles.libraryHeading}><h2>Kütüphane</h2><div><button onClick={() => onCreate('group')}>＋ Grup</button><button onClick={() => onCreate('daily')}>＋ Item</button></div></div><input className={styles.librarySearch} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Aktivite ara…" /><div className={styles.libraryList}>{visible.map((item) => <article key={item.id}><button className={styles.libraryIdentity} onClick={() => onEdit(item)}><i style={{ background: item.color ?? palette[0] }} /><span><strong>{item.name}</strong><small>{groups.find((group) => group.id === item.group_id)?.name ?? 'Grupsuz'} · {item.kind === 'metric' ? 'Ölçüm' : item.activity_tag || (item.kind === 'persistent' ? 'Geçici' : 'Aktivite')}{reminders.some((reminder) => reminder.item_id === item.id && reminder.is_enabled) ? ' · 🔔' : ''}</small></span></button><div>{item.kind !== 'metric' && <button onClick={() => onAddToday(item)}>Bugün</button>}<button className={item.is_in_plan ? styles.inPlan : ''} onClick={() => void onTogglePlan(item)}>{item.is_in_plan ? 'Planda ✓' : 'Plana ekle'}</button></div></article>)}</div></section></div>;
}
