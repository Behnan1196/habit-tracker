...
export function PlannerShell({ user }: { user: User }) {
  ...
  return <div className={styles.app}>
    ...
    <main className={styles.main}>
      ...
      <header className={styles.header}><div><p>Ritmini bul</p><h1>Momentum</h1></div><div className={styles.headerActions}><Link className={styles.analyticsLink} href="/analytics"><span>⌁</span> Analitik</Link><button className={styles.focusButton} onClick={() => setFocusOpen(true)}><span className={styles.focusDot} /> Bugünün Planı <b>{planned.length}</b></button></div></header>
