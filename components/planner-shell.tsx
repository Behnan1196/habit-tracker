...
export function PlannerShell({ user }: { user: User }) {
  ...
  return <div className={styles.app}>
    <main className={styles.mainWide}>
      ...
      <div className={styles.stickyTop}><header className={styles.header}><div className={styles.compactBrand}><span>M</span><strong>{workspace === 'agenda' ? 'Ajanda' : workspace === 'library' ? 'Kütüphane' : workspace === 'modules' ? 'Modüller' : 'Ayarlar'}</strong></div><div className={styles.headerActions}>{workspace === 'agenda' && <button className={styles.focusButton} onClick={() => openAgenda('focus')}><span className={styles.focusDot}>●</span> Kalanlar <b>{focusPlannedCount}</b></button>}<AppMenu user={user} active="calendar" view={view} onViewChange={workspace === 'agenda' ? changeView : undefined} notificationPermission={notificationPermission} onEnableNotifications={() => void enableNotifications()} onManageSlots={() => setSlotManagerOpen(true)} /></div></header></div>
      ...
      {workspace === 'settings' && <section className={styles.surfacePanel}><span>⚙</span><h2>Ayarlar</h2><p>Günün yapısını, bildirimleri ve ileride bağlantılı servisleri buradan yöneteceksin.</p><div className={styles.settingsList}><button onClick={() => setSlotManagerOpen(true)}><span>◷</span><strong>Zaman dilimleri</strong><small>Saatleri ve günün bölümlerini düzenle</small><b>›</b></button><button onClick={() => void enableNotifications()}><span>◉</span><strong>Bildirimler</strong><small>{notificationPermission === 'granted' ? 'Bildirimler açık' : 'Bildirim iznini yönet'}</small><b>›</b></button></div></section>}
    </main>
